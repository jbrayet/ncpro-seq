...
#The script is used to process bam files: group alignments which have the same read sequence.
...
while(<$bam_line>){
	...
	my ($cur_name,$cur_flag,$mark,$cur_seq)=($1,$2,$3,$4);
	next if ($read_name{$cur_name} || ($mark eq "*"));
