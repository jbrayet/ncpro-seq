...
writeCenterString(c,220,"Version : 1.6.5","Times-Roman",18)
...
for line in fileAnnot : 
	...
	if tab[0] == "piRBase" : 
		writeCenterString(c,140,"piRBase version : "+tab[1].replace("\n",""),"Times-Roman",18)
...
for annotationFile in dargs["annotationFiles"]:
	if "miRNA" in annotationFile or "mirna" in annotationFile:
		tabFlag[0] = 1
	if "rfam" in annotationFile:
		tabFlag[1] = 1
	if "rmsk" in annotationFile:
		tabFlag[2] = 1
	if "coding_gene" in annotationFile:
		tabFlag[3] = 1
	if "pirna" in annotationFile or "piRNA" in annotationFile:
		tabFlag[4] = 1
...
if tabFlag[4] == 1 :
	writeLeft_Tab_String(c,430,"- piRNA file : yes","Times-Roman",12)
else : 
	writeLeft_Tab_String(c,430,"- piRNA file : no","Times-Roman",12)
...
if os.path.isfile(dargs["picDirection"]+"/plotGenomeMapping.png"):
	pageNumber = c.getPageNumber()
...
if os.path.isfile(dargs["picDirection"]+"/plotGenomeMappingDistinctReadSize.png"):
	pageNumber = c.getPageNumber()
...
if os.path.isfile(dargs["picDirection"]+"/plotReadAnnOverview.png"):
	pageNumber = c.getPageNumber()
...
if os.path.isfile(dargs["picDirection"]+"/plotmiRNAmapping.png"):
	pageNumber = c.getPageNumber()
...
if os.path.isfile(dargs["picDirection"]+"/plotRfamClassOverviewExp.png"):
	pageNumber = c.getPageNumber()
...
if os.path.isfile(dargs["picDirection"]+"/plotRmskClassOverviewExp.png"):
	pageNumber = c.getPageNumber()
