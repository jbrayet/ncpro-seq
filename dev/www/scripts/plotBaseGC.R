...
plot.save <- ggplot(gc.matrix.ggplot, aes(x=idx, y=value, colour=variable)) + geom_line() + geom_point() + theme_bw() + theme(axis.title=element_text(face="bold")) + xlab("Position in reads") + ylab("GC content") + xlim(gc.size[1],gc.size[length(gc.size)]) + ylim(0,100) + scale_colour_brewer(palette="Set1") + guides(color=guide_legend(title="Samples"))
