...
read ans
if [ XX${ans} = XXy ]; then
    $get bowtie-0.12.8-src.zip http://sourceforge.net/projects/bowtie-bio/files/bowtie/0.12.8/bowtie-0.12.8-src.zip/download?use_mirror=freefr
...
if [ XX${ans} = XXy ]; then
    ...
    #From sources
    $get libpng-1.5.12.tar.gz http://www.imagemagick.org/download/delegates/libpng-1.5.12.tar.gz
    tar -xvf libpng-1.5.12.tar.gz
    cd libpng-1.5.12
